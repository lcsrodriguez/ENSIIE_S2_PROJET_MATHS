...
v_1_h_N = (1/(1 + r_N))*(f(s*(1 + h_N)**2)*q_N +
                         f(s*(1 + h_N)*(1 + b_N))*(1 - q_N))
v_1_b_N = (1/(1 + r_N))*(f(s*(1 + h_N)*(1 + b_N))
                         * q_N + f(s*(1 + b_N)**2)*(1 - q_N))
...
beta_1 = (1/((b_N - h_N)*((1 + r_N)**2))) * \
    (f((1 + h_N)*S)*(b_N + 1) - f((1 + b_N)*S)*(1 + h_N))
...
def price3(n, s, r, sigma, T, f):
    ...
    prix_mc = 0
    for i in range(n):
        prix_mc += np.exp(-r*T)*f(s*np.exp((r - ((sigma**2)/2))
                                           * T + sigma*np.sqrt(T)*ksi[i]))
    prix_mc *= (1/n)
    return prix_mc
...
plt.savefig("figs/" + "evol_pricer3.png")
plt.show()
...
plt.legend()
#plt.savefig("figs/" +"compaPUT_P3.png")
plt.show()
...
plt.title("Comparaison du PUT et du pricer3")
plt.legend()
#plt.savefig("figs/" +"compaPUT_P3_conv2.png")
plt.show()
...
#plt.savefig("figs/" +"18_graph3D_put.png")
plt.show()
...
plt.plot(N_liste, put(s, r, sigma, T, K) *
         np.array([1 for k in range(1, 101)]), color='green', label='put')
plt.legend()
plt.xlabel("$N$")
plt.ylabel("Valeur du pricer")
plt.title("Prix de l'option via le pricer 2 par rapport au put en fonction de $N$")
#plt.savefig("figs/" +"convergence_f_max.png")
plt.show()
...
plt.plot(X_liste, np.fmax(K - np.exp(X_liste), 0),
         color="blue", label="payoff")
...
p = ax.plot_surface(X_liste_m, T_liste_m, P,
                    cstride=1, linewidth=0, cmap='jet')
cb = fig.colorbar(p, ax=ax)
plt.title("METHODE EXPLICITE\nPut de l'option en fonction de $x$ et $t$")
#plt.savefig("figs/" +"E_3D.png")
plt.show()
...
plt.title("METHODE EXPLICITE\nValeurs de $p(t,x)$")
plt.colorbar()
plt.savefig("figs/" + "E_headmap.png")
plt.show()
...
plt.title("implicite")
plt.colorbar()
plt.savefig("figs/" + "I_heatmap.png")
plt.show()
